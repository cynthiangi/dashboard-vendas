...
with aba1:
    ...
    with coluna1:
        ...
        st.plotly_chart(fig_receita_estados, use_container_widht=True)
    with coluna2:
        st.metric('Quantidade de vendas', formata_numero(dados.shape[0]))
        st.plotly_chart(fig_receita_mensal, use_container_widht=True)
        st.plotly_chart(fig_receita_categorias, use_container_widht=True)

with aba2:
    coluna1, coluna2 = st.columns(2)
    with coluna1:
        st.metric('Receita', formata_numero(dados['Preço'].sum(), 'R$'))
        st.plotly_chart(fig_mapa_vendas, use_container_widht=True)
        st.plotly_chart(fig_vendas_estados, use_container_widht=True)
    with coluna2:
        st.metric('Quantidade de vendas', formata_numero(dados.shape[0]))
        st.plotly_chart(fig_vendas_mensal, use_container_widht=True)
        st.plotly_chart(fig_vendas_categorias, use_container_widht=True)
with aba3:
